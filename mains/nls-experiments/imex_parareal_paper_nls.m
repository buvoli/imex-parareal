...
%% -- Reference Solution -----------------------------------------------------------------------------------------------
erk_options = struct('coeffGenerator', @ERK4, 'parameters', pars,'max_ts_to_store', 2);
Nt_ref = 75000;
[TS, YS, tcpu_erkG, tccpu_erkG] = ERK(LF, NF, tspan, y0, Nt_ref, erk_options); 
y_ref = YS(:, end);
...
for i = 1 : length(Nts)
    ...
    xlabel('stepsize $h$', 'interpreter', 'latex'); ylabel('relative error', 'interpreter', 'latex');
